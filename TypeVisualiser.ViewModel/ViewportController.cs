...
namespace TypeVisualiser.UI
{
    public class ViewportController : TypeVisualiserControllerBase, IDiagramController, IDiagramCommandsNeedsRefactor
    {
        ...
        public void LoadDiagramDataFromFile(object deserializedDataFile)
        {
            ...
            var typesNoLongerInDiagram = new List<IVisualisableTypeData>();
        ...
        public void ShowLineDetails(DiagramElement arrowheadOrLine)
        {
            ...
            //new UsageDialog().ShowDialog("Resources.ApplicationName", Subject.Name, Subject.Modifiers.TypeTextName, pointingAtAssociation);
            this.Factory.GetInstance<IShowDialog>().ShowUsageDialog(Subject, pointingAtAssociation);
        }
        ...
        private static IVisualisableTypeData FindTypeInFile(TypeVisualiserLayoutFile data, string id)
        {
            ...
            if (subject.Parent != null && subject.Parent.AssociatedTo.Id == id)
            {
                return data.Subject;
            }

            AssociationData type = subject.Associations.FirstOrDefault(x => x.AssociatedTo.Id == id);
            if (type != null)
            {
                return type.AssociatedTo;
            }

            type = subject.Implements.FirstOrDefault(x => x.AssociatedTo.Id == id);
            if (type != null)
            {
                return type.AssociatedTo;
            }
